...
function joinRoom(socket, room, name){
  ...
  io.to(room.code).emit('chatMessage',{
    playerId:null,name:'Server',message:`${room.players[socket.id].name} joined the world.`,
  ...
  socket.on('disconnect',()=>{
    for(const [code,r] of rooms){
      if(r.players[socket.id]){
        const leavingName=r.players[socket.id].name||'A player';
        delete r.players[socket.id];
        socket.to(code).emit('playerLeft',socket.id);
        io.to(code).emit('chatMessage',{
          playerId:null,name:'Server',message:`${leavingName} left the world.`,
          time:Date.now(),system:true
        });
